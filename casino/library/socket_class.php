...
class PHPWebSocket
{
	...
	const WS_STATUS_TIMEOUT = interval_time;
	...
	function wsStartServer($host, $port) {
		...
		$server_details = file_get_contents(__DIR__ . '../../details.php');
		$server_details = explode('|', $server_details);
		$server_name = $server_details[0];
		$server_ip = $server_details[1];
		$time = time();

...
{oInit={};}
var camelToHungarian=$.fn.dataTable.camelToHungarian;if(camelToHungarian){camelToHungarian(ColVis.defaults,ColVis.defaults,true);camelToHungarian(ColVis.defaults,oInit);}
...
this.dom.buttons.splice(0,this.dom.buttons.length);this.dom.groupButtons.splice(0,this.dom.groupButtons.length);if(this.dom.restore){this.dom.restore.parentNode(this.dom.restore);}
...
this.s.dt.aoDrawCallback.push({"fn":function(){that._fnDrawCallback.call(that);},"sName":"ColVis"});$(this.s.dt.oInstance).bind('column-reorder.dt',function(e,oSettings,oReorder){for(i=0,iLen=that.s.aiExclude.length;i<iLen;i++){that.s.aiExclude[i]=oReorder.aiInvertMapping[that.s.aiExclude[i]];}
var mStore=that.s.abOriginal.splice(oReorder.iFrom,1)[0];that.s.abOriginal.splice(oReorder.iTo,0,mStore);that.fnRebuild();});$(this.s.dt.oInstance).bind('destroy.dt',function(){$(that.dom.wrapper).remove();});this._fnDrawCallback();},"_fnApplyCustomisation":function(init)
...
$.fn.dataTableExt.iApiIndex=oldIndex;if(that.s.fnStateChange!==null)
...
{ColVis.aInstances[i].fnRebuild();}}};ColVis.defaults={active:'click',buttonText:'Show / hide columns',aiExclude:[],bRestore:false,sRestore:'Restore original',bShowAll:false,sShowAll:'Show All',sAlign:'left',fnStateChange:null,iOverlayFade:500,fnLabel:null,bCssPosition:false,aoGroups:[],order:'column'};ColVis.aInstances=[];ColVis.prototype.CLASS="ColVis";ColVis.VERSION="1.1.2";ColVis.prototype.VERSION=ColVis.VERSION;if(typeof $.fn.dataTable=="function"&&typeof $.fn.dataTableExt.fnVersionCheck=="function"&&$.fn.dataTableExt.fnVersionCheck('1.7.0'))
